...
import { readableFileExists, isDirectory } from "~/lib/node.server";
...
async function getPublishedBlogPostsMarkdown(): Promise<Array<BlogPost>> {
	let blogDirectoryContents = (await fsp.readdir(blogPath)).map((p) =>
...
export async function getBlogPostMarkdown(
	...
	try {
		contents = (await fsp.readFile(filePath)).toString();
	} catch (e) {
		return null;
	}
